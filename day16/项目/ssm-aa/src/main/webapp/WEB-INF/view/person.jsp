...
<body class="gray-bg">
	...
	<!-- 添加银行对话框 -->
	<div class="modal fade" id="modal_user_addBank" role="dialog" aria-labelledby="modal_user_addBank" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_addBank" method="post" action="addBank.htm">
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-primary"  id="submit_form_user_addBank">保存</button>
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								...
								<td>
									...
									value="豪车" aria-required="true" required/>豪车
									<input type="checkbox" id="pLike2" name="pLike"
									value="名表" aria-required="true" required/>名表
									<input type="checkbox" id="pLike3" name="pLike"
									value="名酒" aria-required="true" required/>名酒
								</td>
								<td>&nbsp;&nbsp;</td>
								<td>存款人性别：</td>
								<td>
								    <input type="radio" id="pSex1" name="pSex"
							...
							<tr>
								...
								<td>&nbsp;&nbsp;</td>
								<td>存款日期：</td>
								<td> <input readonly="readonly" id="createtime" name="createtime" class="laydate-icon form-control layer-date"/></td>
							</tr>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
						   <form id="form_import"action="importPerson.htm" method="post" enctype="multipart/from-data" >
						    <input type="file" name="imp">
						   </form>
				</div>
				<div class="modal-footer">
				<button type="button" class="btn btn-danger"  id="del_user_import">导入</button>
				<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
			</div>
			</div>
		</div>
	...
	$(function () {
	    ...
	    $("#del_user_import").click(function(){
	    	$("#form_import").submit();
	    });
	    
	    //添加银行
	    var validator = $("#form_addBank").validate({
    		submitHandler: function(form){
   		      $(form).ajaxSubmit({
   		    	dataType:"json",
   		    	success: function (data) {
	    ...
	    $("#submit_form_user_addBank").click(function(){
	    	$("#form_addBank").submit();
	    });
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	$("#btn_addBank").click(function(){
	    			$('#modal_user_addBank').modal({backdrop: 'static', keyboard: false});
	    			$("#modal_user_addBank").show();
	        });
	    	//导入弹框
	    		...
	    		var myEcharts = echarts.init(document.getElementById("main"));
	    			...
	    			$.ajax({
	    			    ...
	    			    success:function(res){
	    			    	...
	    			    	ydata = new Array();
	    			    	if(res.success){
	    			    	res.klist.forEach(function(personVo){
	    			    		xdata.push(personVo.bname);
	    			    		ydata.push({value:personVo.num,name:personVo.bname});
	    			    	});
	...
	function initEditUser(getSelection){
		...
		$('#pLike3').prop("checked",getSelection.pLike.indexOf("名酒")!=-1);
